...
TEST(CircleMaxPoolWithArgMaxTest, constructor_P)
{
  ...
  ASSERT_EQ(false, maxpoolwithargmax_node.include_batch_in_index());
}
...
TEST(CircleMaxPoolWithArgMaxTest, input_NEG)
{
  ...
  maxpoolwithargmax_node.include_batch_in_index(true);
  ASSERT_NE(false, maxpoolwithargmax_node.include_batch_in_index());
}
